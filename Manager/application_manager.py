...
import psutil
import pyautogui
from Manager.helper_eyewear_detection import HelperEyewearDetection
from Manager.helper_base import HelperBase
from Manager.helper_path import HelperPath
from Manager.helper_license_mechanism import HelperLicenseMechanism
from Manager.helper_head_pose_info import HelperHeadPoseInfo
from Manager.helper_face_recognition_add_person import HelperFaceRecognitionAddPerson
...
class ApplicationManager:
    ...
    def __init__(self):
        ...
        self.helper_eyewear_detection = HelperEyewearDetection()
        self.helper_path = HelperPath()
        self.helper_license_mechanism = HelperLicenseMechanism()
        self.helper_head_pose_info = HelperHeadPoseInfo()
        self.helper_face_recognition_add_person = HelperFaceRecognitionAddPerson()
    ...
    def get_my_test_link():
        return TestLinkHelper(TESTLINK_API_PYTHON_SERVER_URL, ApplicationManager.get_testlink_api_python_devkey(user_name)).connect(TestlinkAPIGeneric)

    def run_codriver_sample(self, returncode):
        path_to_execudable = None
        if platform_name == "Linux":
            path_to_execudable = self.get_helper_path().get_path_to_codriver_directory_linux() + "/samples/codriver_sample"
    ...
    def run_make_codriver_sample():
        path_to_execudable = None
        if platform_name == "Linux":
            path_to_execudable = HelperPath.get_path_to_codriver_directory_linux() + "/samples/"
        elif platform_name == "Windows":
            pass
        return subprocess.Popen(["make", "codriver_sample"], stdout=subprocess.PIPE, stdin=subprocess.PIPE, cwd=path_to_execudable)
    ...
    def get_helper_eyewear_detection(self):
        return self.helper_eyewear_detection

    def get_helper_path(self):
        return self.helper_path

    def get_helper_license_mechanism(self):
        return self.helper_license_mechanism

    def get_helper_head_pose_info(self):
        return self.helper_head_pose_info

    def get_helper_face_recognition_add_person(self):
        return self.helper_face_recognition_add_person
